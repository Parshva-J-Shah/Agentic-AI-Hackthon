        ...
        if request.candidate_name:
            for cand in ranked:
                if cand.get("name", "").lower() == request.candidate_name.lower():
                    selected = cand
                    break
        if selected is None and request.alternative_id:
            for cand in ranked:
                if (
                    cand.get("id") == request.alternative_id
                    or cand.get("activity_id") == request.alternative_id
                    or cand.get("name", "").lower() == request.alternative_id.lower()
                    or request.alternative_id.lower() in cand.get("name", "").lower()
                ):
                    selected = cand
                    break
        if selected is None:
