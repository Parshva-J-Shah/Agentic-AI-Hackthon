...
def _synthesize_reply(
    ...
    if intent in {"DISRUPTION", "REPLAN"}:
        ...
        delta_text = (
            f"saving €{abs(b_delta):.2f}"
            if b_delta < 0
            else (f"adding €{b_delta:.2f}" if b_delta > 0 else "no change in cost")
        )
        sim_note = "[SIMULATION PREVIEW] " if simulate else ""

        reply = (
            f"{sim_note}I detected the disruption regarding '{removed_name}'. "
            f"I analyzed your schedule and selected '{added_name}' (€{added_cost:.2f}) as the optimal replacement. "
            f"All timing constraints and conflict checks passed. "
            f"Your trip budget has been updated from €{b_before:.2f} to €{b_after:.2f} ({delta_text})."
        )
        if simulate:
